...
//            UserPosition userPosition = new UserPosition(id, xPos, yPos);
            //return userPositionRepo.save(userPosition);
            return null;
        };
    }
    ...
    public DataFetcher updatePosition() {
        return dataFetchingEnvironment -> {
            ...
            user.getPosition().setX_pos(xPos);
            user.getPosition().setY_pos(yPos);
            return userRepo.save(user);
        };
    }
}